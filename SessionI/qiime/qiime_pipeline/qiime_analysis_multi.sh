...
mkdir tree
biom convert -i otus/otu_table_cc.biom -o otus/otu_table_te.txt --to-tsv --header-key taxonomy --output-metadata-id "Consensus Lineage" 
...
#Pick closed reference OTU for PiCRUST metagenome prediction
echo "pick_otus:enable_rev_strand_match True"  >> otus/picrust_otu_picking_params_97.txt
echo "pick_otus:similarity 0.97" >> otus/picrust_otu_picking_params_97.txt
pick_closed_reference_otus.py -i seq.fna -o otus/picrust97/ -p otus/picrust_otu_picking_params_97.txt -r /usr/local/lib/python2.7/dist-packages/qiime_default_reference/gg_13_8_otus/rep_set/97_otus.fasta -t /usr/local/lib/python2.7/dist-packages/qiime_default_reference/gg_13_8_otus/taxonomy/97_otu_taxonomy.txt
